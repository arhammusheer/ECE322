...
void eval(char *cmdline)
{
    ...
    else if (pid == 0)
    {
        ...
        {
            printf("%s: Command not found \r ", argv[0]);
            exit(0);
        }
...
int builtin_cmd(char **argv)
{
    ...
    char builtin[5][10] = {"quit", "fg", "bg", "jobs"};
    int i;
...
void do_bgfg(char **argv)
{
    ...
    // If argv[0] is "bg", then the job should be sent to the background.
    if (strcmp(argv[0], "bg") == 0)
    {
        // If argv[1] is a PID, then the job with that PID should be sent to the background.
    }
...
void sigchld_handler(int sig)
{
    ...
    // If a child terminates, it should be removed from the job list.
    // If a child stops, it should be marked as stopped.
    // If a child is stopped, it should be marked as stopped.
    // If a child is continued, it should be marked as running.
    // If a child is terminated, it should be removed from the job list.
...
void sigint_handler(int sig)
{
    ...
    // If there is no foreground process group, do nothing.
...
void sigtstp_handler(int sig)
{
    ...
    // If there is a foreground job, send it a SIGTSTP signal.
    // If there is no foreground job, do nothing.
